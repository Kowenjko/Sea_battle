...
class Ship {
    ...
    creatShip(class_ship, class_generic, class_put) {
        ...
        for (let i = this.start; i < this.start + direct; i += step) { //Добавляємо класс основного корабля
    ...
    createBound_x(class_chip, class_bound, bound_ship) {
        let end_b = (this.direction == 'v') ? this.start - 8 : this.start + this.desk - 9; // закынчення межі
        ...
        for (let i = this.start - start_x; i < end_x; i += 1) { //Добавляємо межі по х
            if (i >= 0) {
                document.querySelector(`.${class_chip}-${i}`).classList.add(class_bound); // верхня межа
                document.querySelector(`.${class_chip}-${i}`).classList.add(bound_ship); // верхня межа
                // arr.push(i);
            }
            if (i + step_x <= 99) {
                document.querySelector(`.${class_chip}-${i + step_x}`).classList.add(class_bound); // нижня межа
                document.querySelector(`.${class_chip}-${i + step_x}`).classList.add(bound_ship); // нижня межа
                // arr.push(i + step_x);
            }
        }
    }
    ...
    createBound_y(class_chip, class_bound, bound_ship) {
        ...
        for (let i = this.start; i < this.start + direct_y; i += step) { //Добавляємо межі по у
            if (i % 10) {
                document.querySelector(`.${class_chip}-${i - 1}`).classList.add(class_bound); // ліва межа
                document.querySelector(`.${class_chip}-${i - 1}`).classList.add(bound_ship); // ліва межа                
            }
            if ((i % 10) - 9 != 0) {
                document.querySelector(`.${class_chip}-${i + end_y}`).classList.add(class_bound); // права межа
                document.querySelector(`.${class_chip}-${i + end_y}`).classList.add(bound_ship); // права межа              
            }
        }
    }
}
...
function createPut() {
    ...
    while (num.length < 100) {
        ...
        if (!found) { num[num.length] = randomNumber; }
    }
    return num;
}
...
function removeClass(where, class_add) {
    where.forEach((item, i) => {//удаляємо всі класи для повторної розтановки
        item.className = '';
...
function avtoCreate(where, choice) {
    // console.log(where)
    if (choice == 'avto_comp') { removeClass(where, 'd-comp') }
    else { removeClass(where, 'd') }
    ...
    for (let i = 3; i >= 0; i--) { // роставляємо кораблі компютера
        // console.log('---------------------------')
        for (let j = 0; j < arr_ships[i]; j++) {
            ...
            if (i == 1 && ((numReserve_comp[j] > 79 && rand_pos == 0) || ((numReserve_comp[j] % 10 > 7) && rand_pos == 1))) {
                numReserve_comp.push(numReserve_comp.shift())
                console.log('remove 2')
                create_ship = false;
            }
            // перевіряємо чи 3-палубний розмущується в зоні
            else if (i == 2 && ((numReserve_comp[j] > 69 && rand_pos == 0) || ((numReserve_comp[j] % 10 > 6) && rand_pos == 1))) {
                numReserve_comp.push(numReserve_comp.shift())
                console.log('remove 3')
                create_ship = false;
            }
            // перевіряємо чи 4-палубний розмущується в зоні
            else if (i == 3 && ((numReserve_comp[j] > 59 && rand_pos == 0) || ((numReserve_comp[j] % 10 > 5) && rand_pos == 1))) {
                numReserve_comp.push(numReserve_comp.shift())
            ...
            // Якщо все вірно будуємо корабль
                ...
                (arr.some(elem => elem > 0)) ? create_ship = false : create_ship = true;//перевіряємо чи частина корабля не песікається з ішим
            }
            if (create_ship) {
                ...
                if (choice == 'avto_comp') {
                    ...
                    cor.creatShipComp(`desk-${i + 1}-ship-${j + 1}`, 'ship-put-comp', 'put');
            ...
            else { j--; create_ship = false; index++; }

            console.log('index=' + index);
            console.log('index_melting=' + index_melting);
            if (index_melting > 9) {
                console.log('Розтановка закінчена');
                if (choice == 'avto_comp') { games_start_comp = true; }
                if (choice == 'avto_user') { games_start_user = true; }
            }
            if (index > 20) { console.log('Розтановка не получилась'); avtoCreate(where, choice); }
            // else { console.log('Розтановка закінчена'); }

            where.forEach((item, i) => {

                if (item.classList.contains('bound') || item.classList.contains('bound') || item.classList.contains('ship-put') || item.classList.contains('ship-put-comp')) {
                    arr_bord.push(i);
                }
            })
...
function putBound(start, desk, direction) {
    ...
    for (let i = start; i < start + direct; i += step) { //Добавляємо класс основного корабля
        arr_lend.push(i);
    }
    return arr_lend;
}
...
function shipHit(class_field, index, id_shot) {
    let desk_1_ship_1 = document.querySelectorAll(`.${class_field} .desk-1-ship-1`);
    let desk_1_ship_2 = document.querySelectorAll(`.${class_field} .desk-1-ship-2`);
    let desk_1_ship_3 = document.querySelectorAll(`.${class_field} .desk-1-ship-3`);
    let desk_1_ship_4 = document.querySelectorAll(`.${class_field} .desk-1-ship-4`);
    let desk_2_ship_1 = document.querySelectorAll(`.${class_field} .desk-2-ship-1`);
    let desk_2_ship_2 = document.querySelectorAll(`.${class_field} .desk-2-ship-2`);
    let desk_2_ship_3 = document.querySelectorAll(`.${class_field} .desk-2-ship-3`);
    let desk_3_ship_1 = document.querySelectorAll(`.${class_field} .desk-3-ship-1`);
    let desk_3_ship_2 = document.querySelectorAll(`.${class_field} .desk-3-ship-2`);
    let desk_4_ship_1 = document.querySelectorAll(`.${class_field} .desk-4-ship-1`);
    let put = document.querySelectorAll(`.${class_field} .put`);
    //   ------------------------------------------------------------
    if (put.length == 0) {// якщо підбиті всі кораблі 
        if (class_field == 'wrapper') {
        ...
        if (index.classList.contains('ship-put-comp')) {
            ...
            if (class_field == 'wrapper') { shot_games = true; }
            if (class_field == 'wrapper-comp') { shot_games = false; }
        }
